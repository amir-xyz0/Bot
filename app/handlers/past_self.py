...
# ============================================================
# ۳. دریافت پاسخ کاربر (با شرط حالت مصاحبه)
# ============================================================
...
async def finish_interview(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if user:
            ...
            db.commit()
            logger.info(f"✅ پاسخ‌های {user_id} ذخیره شد (تعداد: {len(answers)})")
...
async def delete_answers(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if user:
            ...
            db.commit()
            logger.info(f"🗑️ پاسخ‌های {user_id} پاک شد")
    except Exception as e:
